...
### ٥. القوائم تعود كاملةً — لا ترقيم صفحات

`GET /candidates` و`/reports` و`/evaluations` و`/users` تعيد **كل** ما في نطاقك دفعةً واحدة، بلا `page` ولا `limit`. الشاشة تُرشِّح عندها لا عند الخادم.

ما يعنيه ذلك لك:

- **لا تستدعِ القائمة في حلقة.** استدعاءٌ واحد ثم رشِّح في ذاكرتك.
- **توقّع حمولةً تكبر مع المركز**: عشرون ألف مرشّح ≈ ٤ ميغابايت و~٧٠٠ مللي ثانية للطلب الواحد. اضبط مهلة عميلك على ذلك لا على ثانيةٍ واحدة.
- **الاستثناءان المحدودان**: `/schedules` بنافذةٍ متدحرجة وسقفٍ صلب (٢٠٠٠ صفّ)، و`/audit/log` و`/notifications` مُرقَّمان.
