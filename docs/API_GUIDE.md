...
- **`sort` من قائمة مغلقة**: `code` · `sector` · `rank` · `tier` · `status` · `classification` · `created`. غيرها ٤٢٢.
...
**بقيّة القوائم بلا ترقيم بعد**: `/reports` و`/evaluations` و`/users` تعيد نطاقك كاملاً. و`/schedules` بنافذةٍ متدحرجة وسقفٍ صلب (٢٠٠٠ صفّ)، و`/audit/log` و`/notifications` مُرقَّمان.
