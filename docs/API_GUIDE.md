...
if "candidate.create" not in me["permissions"]:
    raise SystemExit("الحساب لا يملك إضافة المشاركين — راجع مدير النظام")
```
...
المنصّة تعمل بـ**رمز المشارك** (`participantCode`) لا بالاسم. قائمة المشاركين **لا تحوي أسماء لأي دور بلا استثناء** — حتى من يملك صلاحية الأسماء. والبحث فيها بالرمز لا بالاسم.
...
المستخدم المحصور بقطاع لا يخرج منه، وتمريرك `?sectorId=` لقطاع آخر لا يوسّع شيئاً — يضيّق فقط. والمشاركون المصنّفون (`secret` / `top_secret`) محجوبون عمّن لا يملك `candidate.view_classified` في **القوائم والتفاصيل والتجميعات والإحصاءات والتصدير والسجل** معاً، لا في القائمة وحدها.
...
### الوصفة ١ — أضف مشاركاً من نظام خارجي
...
| `rankLabel` | من `GET /ranks` — **الرتبة تقود تصنيف الفئة القيادية**، فنصٌّ حرّ لا يطابق قائمة المركز يُدخِل المشارك في الفئة الخطأ |
| `classification` | `normal` افتراضاً. رفعها فوق ذلك يحجب المشارك عمّن لا يملك التصريح |

الاستجابة تحمل `participantCode` — **احفظه**. هو مفتاحك في كل شاشة ومستند بعد ذلك.

**إن كان المشارك مسجّلاً مسبقاً** يعود ٤٢٢. لا تُعِد المحاولة: ارفع **طلب تحديث** (الوصفة التالية).

### الوصفة ٢ — حدّث بيانات مشارك مسجّل
...
```json
{ "error": "لم يوقّع المشارك ولم يُقرّ بصحّة بياناته" }
```
...
- [ ] مسار الاستقبال مُختبَر بمشارك تجريبي من الوصول إلى الاعتماد
