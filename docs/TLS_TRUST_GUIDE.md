...
> الغرض: إزالة تحذير المتصفّح نهائياً عند فتح منصّة كفاءات على `https://172.16.0.73/`.
...
| **الوصول بعنوان IP** | `server_name` الفعّال على الخادم هو `172.16.0.73`، لذا الرابط المعتمد للمستخدمين هو `https://172.16.0.73/`. الشهادة تغطي أيضاً `kafaat.internal.gov.sa` في SAN، فإن ضُبط DNS داخلي لاحقاً سيعمل الاسمان معاً دون إعادة إصدار. |
