# دليل تنصيب الجذور الداخلية على أجهزة المستخدمين

<!-- جذران: «Tamkeen Alkafaat Internal Root CA» للعنوان العددي، و«Tamkeen MOITP
     Internal Root CA» للاسم المعتمد. الأول وحده لا يكفي بعد اليوم. -->

> الغرض: إزالة تحذير المتصفّح نهائياً عند فتح منصّة كفاءات.
> الرابط المعتمد صار `https://tamkeentp.moitp.gov.sa/` — يحلّه DNS الداخلي
> وتغطّيه شهادة سارية. والعنوان `https://172.16.0.73/` يبقى عاملاً.
>
> ⚠ **جذران لا واحد.** الاسم الجديد موقَّع من جذرٍ ثانٍ اسمه
> `Tamkeen MOITP Internal Root CA`، والعنوان العددي ما زال على الجذر الأول
> `Tamkeen Alkafaat Internal Root CA`. فمن نُصِّب عليه الأول وحده يرى تحذيراً
> على الرابط الجديد. **يلزم تنصيب الجذر الثاني على الأجهزة قبل تعميم الرابط.**
...
- شهادة الاسم الجديد صدرت: الموضوع `tamkeentp.moitp.gov.sa`، وSAN يحمل الاسم والعنوان معاً، والمُصدِر هو الجذر الثاني.
...
| **الرابط الجديد يعمل — بجذرٍ ثانٍ** | حُلَّ القيد بإنشاء جذرٍ مستقلّ `Tamkeen MOITP Internal Root CA` يسمح قيدُ أسمائه بالنطاق الجديد، لا بإعادة إنشاء الجذر الأول. الاسم القديم يبقى على جذره، والعنوان العددي كذلك — فلا ثقةٌ منصَّبة تُبطَل. **والثمن أن على الأجهزة تنصيب جذرين.** الشهادة الجديدة سارية إلى `2027-09-19` وتحمل في SAN الاسمَ والعنوان معاً. |
| **موقعان في nginx لا موقع واحد** | لا يمكن لكتلة `server` واحدة أن تقدّم شهادتين، فالخادم يفصلهما بـSNI: `/etc/nginx/sites-available/kafaat.conf` للعنوان بالشهادة الأولى، و`tamkeentp.conf` للاسم بالشهادة الثانية. أي توحيدٍ لهما في ملفٍّ واحد يُسقط إحدى الشهادتين. |
| **النطاق المجرّد `moitp.gov.sa` لا يُقصد** | لا يحلّه DNS ولا تغطّيه شهادة. الاسم المعتمد هو `tamkeentp.moitp.gov.sa` وحده — وذكرُ المجرّد في إعدادٍ أو دليل يَعِد بوصولٍ لا وجود له. |
| **DNS** | سجل A لـ`tamkeentp.moitp.gov.sa` ← `172.16.0.73` **قائمٌ ويعمل** على المحلّل الداخلي (مُتحقَّق منه بـ`getent hosts`). |
