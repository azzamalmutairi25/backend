...
| الطريقة | المسار | الصلاحية | الغرض |
|---|---|---|---|
| GET | `/analytics/executive` | `analytics.executive` | **القيادة التنفيذية — المؤشرات**: مؤشرات بفروقات، خريطة حرارية كفاءة×قطاع، اتجاهات، مقارنة قطاعات، مقارنة فئات قيادية، توزيع جاهزية، رؤى تلقائية، والفجوة عن مستهدف الرتبة لكل كفاءة×قطاع بالمستويات (`heatmapGap` مع تغطيتها)، ونسبة الغياب لكل قطاع من الحضور المرصود (`sectorComparison[].absenceRate`). المُعامِل `?months` (٦ افتراضاً) |
| GET | `/analytics/executive/overview` | `analytics.executive` | **القيادة التنفيذية — نظرة شاملة**: ثلاثة عشر قسماً (المشاركون، الموجات، طاقة الجدولة، الجلسات، التسليم للجهات، الاستقبال، الحضور، التقييم، القياس، التقارير، خطط التطوير، الكفاءات، التدقيق) بشكلٍ موحّد `{key,label,icon,route,metrics,bars}`، والعدّاد الذي تقبل شاشتُه فلترَه يحمل `to: {path, query}`. **خارجها عمداً**: الإعدادات، وطلبات تحديث البيانات، والفريق والأدوار |
| GET | `/analytics/executive/reports` | `analytics.executive` | **القيادة التنفيذية — التقارير**: مؤشرات السلسلة، خطّ الاعتماد، أطول انتظار في كل مرحلة، توزيع التوصيات، وأحدث التقارير **بالرمز لا بالاسم** (اطّلاع لا تحرير). ومعدّل الإعادة (`kpis.reworkRate` من التقارير التي دخلت السلسلة)، والمعتمَد بلا ملخّص تنفيذي الأقدم أولاً (`pendingExecSummary`)، وزمن الدورة بالوسيط لكل مرحلة على ١٢ شهراً (`cycleTime`، تقريبيّ من تواريخ الدورة وسجل التدقيق). المُعامِل `?limit` (٢٥ افتراضاً، ١٠٠ حدّاً) |
| GET | `/analytics/executive/today` | `analytics.executive` | **القيادة التنفيذية — بانتظار قرارك**: `today` (جلسات اليوم وحضورها، والبهو، وبيان البوّابة)، و`queue` ما يبتّ فيه القارئ — **كل صفٍّ بصلاحية بتّه لا بالدور** (بيان التصاريح، والاعتماد النهائي، والملخّص التنفيذي، ومسودّات المشاركين، واعتماد الاستقبال، والتقييمات المُرسلة)، و`blockers` ما توقّف عند غيره بعمره وصاحب قراره. كل صفّ `{key,label,count,oldestDays,route,query,linkPerm}` |
