...
مع عزل بوّابة المرشح العامة في الـDMZ.
...
## ⛔ أوقفوا الإطلاق حتى يُصلَح هذا

اختبار الحمل كشف عطلاً يُسقط الخدمة تحت التزامن — وهو غير مُصلَح بعد:

**سباق توليد رمز المشارك.** [`Assessment::generateParticipantCode()`](../app/Models/Assessment.php)
يقرأ أعلى رقم ثم يُدرج بلا قفل. تحت **٨ كتّاب متزامنين فشلت ٣٦٪ من الترشيحات
بـ500** — سجلّ الأخطاء يُظهر ثلاثة طلبات حسبت `LT-266` نفسه فاصطدمت بالقيد الفريد.
وفيه عيبٌ ثانٍ: يجلب **كل** رموز القطاع إلى ذاكرة PHP في كل إدراج، فالكلفة تنمو
مع نجاح النظام (١٠٠٠٠ مرشح ⇒ ١٠٠٠٠ صفّاً لكل ترشيح جديد).

العلاج: تسلسل ذرّي في القاعدة لكل قطاع (`UPDATE … RETURNING` على جدول عدّادات،
أو `SEQUENCE` لكل بادئة) بكلفة ثابتة، مع إعادة محاولة واحدة عند 23505 كشبكة أمان.

> يوم الإطلاق يُضاف عشرات المرشحين دفعةً واحدة من عدّة منسّقين — وهي الحالة
> التي يظهر فيها العطل بالضبط. التفاصيل في [`load-test/README.md`](../load-test/README.md).
...
**لماذا الـDMZ بلا PHP؟** بوّابة المرشح مكشوفة للإنترنت. جعلها nginx يخدم ملفات
...
النسخة **مشفّرة**: تحوي أسماء المرشحين وهوياتهم وسِيَرهم. أرشفة WAL مفعّلة
...
وافتح ملف مرشّح وتأكّد أن اسمه يُقرأ. نسخة لم تُختبَر استعادتها ليست نسخة.
...
الوسيط: كل مستخدمي المنصّة في دلو تقييد واحد، **وكل مرشحي البوّابة يتقاسمون
...
إخفاقات · دخول تجريبي بكل دور · ترشيح مرشّح تجريبي من بوّابة خارجية · تأكّد
...
| 500 عند إضافة مرشحين معاً | **سباق رمز المشارك — العطل أعلاه** |
