...
class SectorBindingTest extends TestCase
{
    ...
    public function test_only_evaluators_assistants_and_discussion_evals_are_sector_bound(): void
    {
        foreach (['EVALUATOR', 'DISCUSSION_EVAL', 'ASSISTANT'] as $r) {
            $this->assertTrue($this->userIn($r, 'PR')->isSectorBound(), $r);
        }
    ...
    public function test_creating_an_evaluator_with_a_sector_succeeds(): void
    {
        $this->actingAsRole('ADMIN');
        $this->postJson('/api/users', [
            'username' => 'ev_ho', 'fullName' => 'مقيّم السجون',
            'roleId' => Role::where('code', 'EVALUATOR')->value('id'),
            'sectorId' => Sector::where('code', 'PR')->value('id'),
            'password' => 'Kafaat@2026', 'userType' => 'external',
        ])->assertCreated();

        $this->assertSame('PR', User::where('username', 'ev_ho')->first()->sector->code);
    }
    ...
    {
        [$c] = $this->makeCandidate(['status' => 'scheduled', 'sectorCode' => 'PR']);
        $this->userIn('EVALUATOR', 'PR');
    ...
    public function test_bound_user_sees_only_their_sector_in_lists(): void
    {
        ...
        foreach ($rows as $r) {
            $this->assertSame('السجون', $r['sectorName']);
        }
    }
    ...
    {
        $as = $this->userIn('ASSISTANT', 'PR');
        $ev = $this->userIn('EVALUATOR', 'DW');
        [$c] = $this->makeCandidate(['status' => 'scheduled', 'sectorCode' => 'DW']);
        $this->actingAsRole('SCHEDULER');
